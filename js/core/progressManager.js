...
import { createProfileId, defaultLearningProfile, normalizeLearningProfile } from "./learningProfileManager.js";
...
export function normalizeProgress(progress = {}) {
  const profile = progress.profile ? {
    ...progress.profile,
    profileId: progress.profile.profileId || progress.learningProfile?.profileId || createProfileId(),
  ...
  const normalized = {
    ...
    matchReviews: Array.isArray(progress.matchReviews) ? progress.matchReviews.slice(0, 5) : [],
    matchReviewXpClaimed: Array.isArray(progress.matchReviewXpClaimed) ? progress.matchReviewXpClaimed : [],
    learningProfile: normalizeLearningProfile(progress.learningProfile, profile),
...
export function createProfileData(data) {
  return {
    ...
    goal: data.goal,
    style: data.style,
    profileId: data.profileId || createProfileId(),
